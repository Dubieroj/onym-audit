...
| `provider/`, `public/discovery/` | **A Discovery provider for audit seats**: Onym's default catalog lists four seat types and no audit seat, and the Discovery contract keeps direct import open — so this site publishes its own Discovery-Static-Ed25519 provider, <https://foldy.io/audit/discovery/manifest.json>, with one catalog, `onym-auditors`, listing this seat and every hub auditor whose manifest verifies. The server signs chained, retained snapshots with the provider key when the listed auditors change or expiry nears; the catalog passes this repository's own 42-check suite, in tests and live |
