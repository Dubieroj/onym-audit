...
**API:** <https://foldy.io/audit/api/> — every address, request and check
...
| `provider/`, `public/discovery/` | **A Discovery provider for audit seats**: Onym's default catalog lists four seat types and no audit seat, and the Discovery contract keeps direct import open — so this site publishes its own Discovery-Static-Ed25519 provider, <https://foldy.io/audit/discovery/manifest.json>, with two catalogs: `onym-auditors`, listing this seat and every hub auditor whose manifest verifies, and `onym-audited-services`, which lists the services from Onym's own catalog that a credited auditor has attested, pinned to their manifests' exact bytes, with each attestation surfaced as the entry `status` (`review` for findings noted, `warning` for a fail). The status links to a page of those attestations. Onym's iOS and Android apps already render that status on a service's row and on its consent screen, so a user who adds this provider sees the audits without any change to the apps. The server signs chained, retained snapshots with the provider key when the entries change or expiry nears; both catalogs pass this repository's own 42-check suite, in tests and live |
| `stellar/`, `public/stellar.js` | **Anchoring in Stellar (testnet)**: the digest of an auditor's own acts in its status list — attestations issued, revoked, superseded; not freshness re-signing, not subjects' replies — is written as the data entry `onym-audit-status` of the auditor's Stellar account, which is the auditor key itself. The app anchors after every issue and revocation, the library shows each register's anchor from a public Stellar node, and `onym-audit anchor [-check]` does the same for this seat. The network keeps every write with its ledger time: a register cannot be backdated or shown differently to different readers unnoticed. Go and JS are pinned to shared vectors; the encoding was accepted by the network |
| `public/hub/`, `hub/`, `netguard/` | **The audit app and hub**: anyone can take the audit seat from a browser at <https://foldy.io/audit/app/> (EN/RU/CNR) — an auditor's cabinet with a pipeline (new orders → held → active → revoked) a **customer cabinet** — auditors searchable and sorted by public facts (orders fulfilled, distinct customers), orders signed with per-order keys, and **requests for proposals** published to every auditor or addressed to one Stellar account, answered with signed offers the orderer chooses among; the list of an identity's orders and requests follows its phrase to any device through a vault on the hub, encrypted with a phrase-derived AES-256-GCM key under a signing key used for nothing else — and a **library** of every attestation the hub serves, searchable by number, component or auditor and verified in the reader's browser. The app keeps only non-extractable keys, signs the manifest and every attestation there, and examines a repository at a commit, any running Onym component by its signed manifest, a Discovery catalog (the 42-check suite, run by the hub), or a build file (hashed by the hub). The hub validates everything against the profile before publishing it under `a/<handle>/`, holds only each auditor's delegated status key, and fetches only public addresses (`netguard` checks the dialed IP). Hub auditors publish signed offers (pro bono or a fixed, verdict-independent price); the hub accepts an order only if it takes an offer exactly, keeps it in an inbox that opens only to the auditor's signed request, checks a commissioned attestation against the countersigned order, and holds a `fail` until the order's embargo ends |
